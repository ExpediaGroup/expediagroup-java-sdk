 ...
 * @property test The payment-sessions call has a test header that can be used to return set responses with the following keywords:<br> * `standard` * `service_unavailable` * `internal_server_error`
 */
@JsonDeserialize(builder = PutCompletePaymentSessionOperationParams.Builder::class)
data class PutCompletePaymentSessionOperationContext(
    val customerIp: kotlin.String,
    val customerSessionId: kotlin.String? =
        null,
    val test: PutCompletePaymentSessionOperationParams.Test? =
        null
) {
    companion object {
        @JvmStatic
    ...
    class Builder(
        ...
        /**
         * @param test The payment-sessions call has a test header that can be used to return set responses with the following keywords:<br> * `standard` * `service_unavailable` * `internal_server_error`
         */
        fun test(test: PutCompletePaymentSessionOperationParams.Test) = apply { this.test = test }

        fun build(): PutCompletePaymentSessionOperationContext {
            validateNullity()

            return PutCompletePaymentSessionOperationContext(
