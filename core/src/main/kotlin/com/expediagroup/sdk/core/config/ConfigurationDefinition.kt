...
class ConfigurationDefinition {
    ...
    private fun define(key: ConfigurationKey): ConfigurationDefinition {
        if (configKeys.containsKey(key.name)) {
            throw ExpediaGroupException(getConfigurationDefinedMultipleTimesMessage(key.name))
        }
    ...
    fun parse(props: Map<String, Any>): Map<String, Any> {
        ...
        if (undefinedConfigKeys.isNotEmpty()) {
            throw ExpediaGroupException(getRequiredConfigurationsNotDefinedMessage(undefinedConfigKeys.joinToString(",")))
        }
