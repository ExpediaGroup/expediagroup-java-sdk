...
function process(filePath: PathOrFileDescriptor): void {
  ...
  const edits = [imports, retrieve, resume, cancel].flatMap((func) => func(root))
  const newSource = root.commitEdits(edits)
  fs.writeFileSync(filePath, newSource)
}

function readRule(ruleName: string): NapiConfig {
  const rule = fs.readFileSync(`./rules/complete-payment-session-links/${ruleName}.yaml`, 'utf-8')
...
export { process as processCompletePaymentSessionLinks }
