...
function readRule (ruleName: string): NapiConfig {
    const rule = fs.readFileSync(`./rules/non-cancellable-date-time-range/${ruleName}.yaml`, 'utf-8')
    return yaml.parse(rule)
}

function changeClassParamType(root: SgNode) : Edit[] {
    const config = readRule("change-class-params-type")
...
function changeBuilderMethodParamType(root: SgNode) : Edit[] {
    const config = readRule("change-builder-method-params-type")
...
export { process as processNonCancellableDateTimeRange }
