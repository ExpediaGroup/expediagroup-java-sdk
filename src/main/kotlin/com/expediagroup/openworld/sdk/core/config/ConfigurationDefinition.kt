...
class ConfigurationDefinition {
    ...
    private fun define(key: ConfigurationKey): ConfigurationDefinition {
        if (configKeys.containsKey(key.name)) {
            throw OpenWorldException(getConfigurationDefinedMultipleTimesMessage(key.name))
        }
    ...
    fun get(name: String): ConfigurationKey =
        configKeys[name] ?: throw OpenWorldException(getConfigurationKeyNotDefinedMessage(name))
    ...
    fun parse(props: Map<String, Any>): Map<String, Any> {
        ...
        if (undefinedConfigKeys.isNotEmpty()) {
            throw OpenWorldException(getRequiredConfigurationsNotDefinedMessage(undefinedConfigKeys.joinToString(",")))
        }
    ...
    private fun parseBoolean(value: Any, name: String): Boolean {
        ...
        throw OpenWorldException(getExpectedNameValueMessage("boolean", name, value))
    }

    private fun parsePassword(value: Any, name: String): ConfigurationKey.Password {
        toPasswordOrNull(value)?.let { return it }

        throw OpenWorldException(getExpectedActualNameValueMessage("string", value.javaClass.name, name, value))
    }
    ...
    private fun parseInt(value: Any, name: String): Int {
        ...
        throw OpenWorldException(getExpectedActualNameValueMessage("32-bit integer", value.javaClass.name, name, value))
    }

    private fun parseDouble(value: Any, name: String): Double {
        toDoubleOrNull(value)?.let { return it }

        throw OpenWorldException(getExpectedActualNameValueMessage("double", value.javaClass.name, name, value))
    }

    private fun parseList(value: Any, name: String): List<*> {
        toListOrNull(value)?.let { return it }

        throw OpenWorldException(getExpectedNameValueMessage("comma-separated list", name, value))
    }
